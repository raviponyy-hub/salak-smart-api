...
async def predict_api(
    ...
    date_time: str = Form(...),
    farm_plot: str = Form(...),
    row: str = Form(...),  # 👈 เพิ่มช่องรับข้อมูล "แถวที่" ให้ครบตามหน้าเว็บ
    tree: str = Form(...)  # 👈 เพิ่มช่องรับข้อมูล "ต้นที่" ให้ครบตามหน้าเว็บ
):
    try:
        ...
        # 5. ส่งผลลัพธ์กลับไปให้ครบตามที่หน้าเว็บ (JavaScript) รอรับไปลงตารางและทำ Excel
        ai_result["date_time"] = date_time
        ai_result["farm_plot"] = farm_plot
        ai_result["row"] = row     # 👈 ส่งค่าแถวกลับไปให้หน้าเว็บ
        ai_result["tree"] = tree   # 👈 ส่งค่าต้นกลับไปให้หน้าเว็บ
